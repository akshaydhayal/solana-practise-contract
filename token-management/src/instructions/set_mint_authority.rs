...
pub fn set_mint_authority(program_id:&Pubkey, accounts:&[AccountInfo])->ProgramResult{
    ...
    let set_mint_authority_ix=set_authority(token_prog.key, 
        mint.key, Some(token_prog.key), AuthorityType::MintTokens,
        user.key, &[user.key])?;
    invoke(&set_mint_authority_ix,
